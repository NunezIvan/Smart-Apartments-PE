...
pair<string, string> separarNombreApellido(const string& nombreCompleto) {
    size_t pos = nombreCompleto.find('_');
    if (pos != string::npos) {
        return make_pair(nombreCompleto.substr(0, pos), nombreCompleto.substr(pos + 1));
    } else {
        return make_pair(nombreCompleto, ""); 
    }
}
...
void CambioPropietario() {
    ...
    while (getline(archivo, linea)) {
        ...
        if (dni_str == DNI_buscar) {
            encontrado = true;
            auto [nombreEncontrado, apellidoEncontrado] = separarNombreApellido(nombreUsuario);
            gotoxy(40, 18);
            ...
            auto [nombreProp, apellidoProp] = separarNombreApellido(nombreUsuario);
            usuario_Apartamento propietario(nombreProp, apellidoProp, nmro_apart, nivel, edificio, dni, false);
